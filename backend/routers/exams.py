...
def _question_bank_for_track(track_id):
    return get_supabase().table("question_bank").select("*").eq("track_id", track_id).execute().data
...
def start_or_resume_exam(req: StartExamRequest, student=Depends(get_current_student)):
    exam = get_supabase().table("exams").select("*").eq("id", req.exam_id).single().execute().data
    ...
    existing = (
        get_supabase().table("exam_sessions").select("*")
    ...
    if existing.data:
        ...
        created = get_supabase().table("exam_sessions").insert({
...
def submit_answer(req: AnswerRequest, student=Depends(get_current_student)):
    ...
    all_answers = get_supabase().table("exam_answers").select("is_correct").eq("session_id", session["id"]).execute().data
    summary = summarize_session(all_answers)
    update["score"] = summary["score"]
    update["max_score"] = summary["max_score"]

    get_supabase().table("exam_sessions").update(update).eq("id", session["id"]).execute()
...
def get_result(session_id: str, student=Depends(get_current_student)):
    ...
    answers = get_supabase().table("exam_answers").select("*").eq("session_id", session_id).execute().data
    summary = summarize_session(answers)
    flags = get_supabase().table("proctor_events").select("id", count="exact").eq("session_id", session_id).execute()
